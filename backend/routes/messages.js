...
router.get('/channel', auth,(req, res) => {
    ...
    query(`select * from (select *,(select username from users where id = sender) as sender_username, (select profile_image from users where id = sender) as pfp from messages where channel = ? order by id desc limit 20) as sub order by id asc`, [req.query.channel_id], (err, result) => {
        if(err){
            console.log(err)
            return err
        }
