...
router.get('/' ,auth,  (req, res) => {
    ...
    where id in (select workspace from workspaces_members where member=?)`, [req.query.id], (err,result) => {
        ...
        if(result.length > 0){
            ...
            query(`select * from channels where workspace = ? and ((public<>'private') or (public='private' and id in (select channel from private_channels_members where member=?)))`, [result[0].id, req.query.id], (err, result) => {
                if(err){
                    console.log(err)
                    return err
                }
